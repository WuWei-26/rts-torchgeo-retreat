...
def predict_and_export_georef(
    ...
    for predictions in predictions_list:
        ...
        bbox_vals = predictions["bbox"]
        minx, maxx, miny, maxy, mint, maxt = bbox_vals
        # if isinstance(bbox_vals, (list, tuple)):
        #     data_bbox = bbox_vals[0]
        # else:
        #     data_bbox = bbox_vals

        col_off = int(round((minx - output_origin_x) / res_x))
        row_off = int(round((output_origin_y - maxy) / res_y))
        ...
        # use overlap-valid pixels only
        valid = predictions["valid_both"][0, 0, :patch_h_clip, :patch_w_clip].numpy().astype(bool)

        # optional: skip very low coverage patches
        coverage = valid.mean() if valid.size > 0 else 0.0
        if coverage < 0.95:
    ...
    print(f"\ntotal prediction batches processed: {batch_idx}")
    print(f"skipped outside ROI: {skipped_outside}")
    ...
    heatmap_masked = heatmap_mean * segment_binary
    ...
    print(f"heatmap raw range: [{heatmap_mean.min():.4f}, {heatmap_mean.max():.4f}]")
    print(f"heatmap masked range: [{heatmap_masked.min():.4f}, {heatmap_masked.max():.4f}]")
    print(f"segment range: [{segment_mean.min():.4f}, {segment_mean.max():.4f}]")
    print(
        f"number of pixels: {(count_arr > 0).sum()} / {H * W} "
        f"({(count_arr > 0).sum() / (H * W) * 100:.1f}%)"
    )
